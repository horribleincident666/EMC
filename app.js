...
function mergeProtectedState(localState, remoteState) {
  ...
  ["formulationHistory", "tasks", "kpi", "longPlans", "ideas", "grn", "stockVariants", "machines", "screenDetails", "mesh", "audit"].forEach(preferLonger);
...
function chooseAdminDisplayName(fallbackPassword = "") {
  return new Promise(resolve => {
    ...
    const finish = async () => {
      ...
      const identityPassword = password.value;
      try {
...
function init() {
  ...
  if (q("#minimizeCallBtn")) q("#minimizeCallBtn").onclick = () => q("#commPanel").classList.add("hidden");
...
function bindActions() {
  ...
  if (q("#dashboardPhotoUpload")) q("#dashboardPhotoUpload").onchange = addDashboardPhotos;
  if (q("#confirmDashboardPhotosBtn")) q("#confirmDashboardPhotosBtn").onclick = confirmDashboardPhotos;
  if (q("#clearDashboardPhotosBtn")) q("#clearDashboardPhotosBtn").onclick = clearDashboardPhotos;
  if (q("#openPhotoManagerBtn")) q("#openPhotoManagerBtn").onclick = openPhotoManager;
  if (q("#closePhotoManagerBtn")) q("#closePhotoManagerBtn").onclick = closePhotoManager;
  ["#photoCropX", "#photoCropY", "#photoCropZoom"].forEach(selector => {
    if (q(selector)) q(selector).oninput = updateSelectedPhotoCrop;
  });
...
function showWelcomeAssistant() {
  ...
  if (!overlay) {
    document.body.insertAdjacentHTML("beforeend", `<div id="welcomeAssistantOverlay" class="modal-overlay hidden"><section class="modal-card welcome-card"><h3 id="welcomeAssistantTitle"></h3><p id="welcomeAssistantLine"></p><div class="modal-actions"><button id="welcomeAssistantClose" type="button">OK</button></div></section></div>`);
...
function renderBlendPlan() {
  ...
  q("#todayBlendTarget").value = state.blendPlan?.target || "";
}

function saveBlendPlan() {
  state.blendPlan ||= {};
  state.blendPlan.plan = q("#todayBlendPlan")?.value || "";
  state.blendPlan.target = q("#todayBlendTarget")?.value || "";
